...
- 基于产品质疑收缩 V1.0.0 MVP：从完整平台回到“给一个 repo，产出 8 个有代码证据的高风险追问点”。
- V1 主线只保留 Survey + 可看提示的 Test；Practice、连续追问、评分、详尽复盘、历史记录先退出成功标准。
- V1 主线移除 `kaomian` / 八股连接和 `bagu-killer`，避免在验证 repo 细节命中率前稀释差异化。
