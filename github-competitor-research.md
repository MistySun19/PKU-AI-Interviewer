...
GitHub 上已经有不少 AI 模拟面试 / 简历面试辅导项目，但真正接近“项目深挖、拷打”的不多。
...
但它们和我们访谈中发现的研究岗实习需求仍有明显差距：它们更偏软件工程项目、GitHub 代码仓库、架构和技术栈；很少覆盖研究岗需要的论文讨论、近期技术流程、带回家任务 / 笔试复盘、项目细节到八股 / 原理的连续追问。
...
- 分析代码仓库，生成项目评分、架构报告、项目专属面试问题和实时模拟面试。
...
- 更像软件工程/代码仓库项目深挖。
- 对研究岗实习生的论文、实验、数据、评测、后训练流程、agent loop、带回家任务复盘没有明显专门建模。
...
- 求职辅助工具，集合简历分析、模拟面试、题目生成器和大模型项目分析器。
...
- 项目深挖似乎更多围绕 README 和问答生成，未看到强连续追问和带回家任务复盘。
- 不太针对中国学生找大厂 / 初创研究岗实习的流程。
...
| --- | --- | --- | --- | --- | --- | --- |
| DevContext.AI | GitHub URL | 是，读代码 | 可选目标岗位 | 有回答评价 / 追问 | 高 | 不够研究岗 |
| Talent Agent | JD + GitHub 仓库 | 是，偏 README / 项目索引 | 是 | 有自适应模拟面试 | 中高 | 可能不够细节级代码深挖 |
| HiredAI | Resume + GitHub username | 是，偏 README/项目验证 | 是 | 有 follow-up Q&A | 中 | 功能较散，深挖不强 |
| Resume Copilot | 全量履历 + JD | 否 | 是 | 否 | 中低 | 预测题，不是真模拟面试 |
| AI Career Copilot | JD + candidate materials | 否/不明显 | 是 | 弱 | 中低 | Q&A preparation |
| FoloUp | JD | 否 | 是 | 有语音追问 | 中低 | 招聘方工具，不深挖项目 |
| 通用模拟面试 | 岗位 / 技术栈 | 否 | 部分支持 | 部分支持 | 低 | 泛题库 |

## 对我们项目的差异化机会

### 机会 1：研究岗实习专用项目拷打

现有项目大多服务软件工程面试。我们可以专门做研究岗实习：

- 用户输入项目经历、论文、岗位方向、目标公司类型。
- AI 生成研究岗风格的深挖：
  - 项目背景
  - 方法选择
  - 实验设计
...
同学 C 和本人案例都显示：初创研究岗会先给真实任务缩小版，二面围绕笔试题继续问。
...
- 大厂：项目 + LeetCode / 代码题 + 八股 + 场景题。
- 初创 / 美国研究岗：项目深挖 + 方向匹配 + 论文 / 近期工作 + 带回家任务。
...
> 面向 AI、算法、研究岗实习候选人的项目深挖压力面试官：根据你的项目、目标岗位和公司类型，模拟真实面试官从项目细节一路追问到原理、论文、技术流程和业务落地。

最小可行产品闭环：

1. 用户输入目标岗位方向：大模型/agent/RL/eval/生成。
2. 用户输入一个项目经历。
3. 用户选择公司类型：大厂 / 初创 / 海外研究岗。
4. AI 提取 5 个高风险追问点。
5. 进入 3 层追问：
   - 项目事实
   - 实现/原理细节
